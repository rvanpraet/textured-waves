...
function drawLine(props) {
  ...
  const drawFn = (x, y) => {
    ...
    // Weight probability based on x position
    const probXMap = easeInOutQuart(map(x, -width * 0.5, width * 0.5, -1, 1) + sin(frameCount * 2) * width * 0.000005);
    const invertedProbXMap = 1 - abs(probXMap);

    // Weight probability based on y position
    const probYMap = easeInOutSine(map(y, -height * 0.5, height * 0.5, 1, 0));
    ...
    if (chance(probabilityFinal)) {
      ...
      // Base offset values
      yOffset = 50 * noiseOffsetStrength;
      xOffset = 50 * noiseOffsetStrength;

      // Offsets based on geometry
      let sinOffset = sin(rowStepCount * 0.0005 + radians(phase));
      let cosOffset = cos(rowStepCount * 0.025);

      // Offsets based on noise
      let xOff = useNoise ? map(noiseVal, 0, 1, -1, 1) * xOffset * 1 : 0;
      let yOff = useNoise ? map(noiseVal, 0, 1, -1, 1) * yOffset * 1 : 0;
      ...
      if (isGradient) {
        // fillColor = lerpColor(color())
        fillColor = gradientProb < 0 ? prevColor : nextColor;
        fillColor = lerpColor(colorRGB(currentColor), colorRGB(fillColor), gradientMap);
      }

      //   const alpha = noiseVal * random(alphaRnd) * 2;
      const alphaGradientMult = isGradient ? 0.5 : 1;
      const alpha = 1 * alphaGradientMult;
      const size = random(weightRnd * 0.25, weightRnd * 1.25);

      push();
      noStroke();

      //   if (isGradient) {
      //     fillColor = lerpColor(color(prevColor), color(nextColor), map(noiseVal, 0, 1, 0, 1));
      //   }

      fill(colorAlpha(fillColor, alpha));
      circle(posX, posY, size * 0.35);
...
/**
 * Turns a P5 color value (hex, decimal) into an RGB color value
 * @param {t} P5.Color - P5 color value
